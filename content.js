...
var final_transcript = '';
  ...
  if(final_transcript.toLowerCase().indexOf("selam ver")>-1  )
  {
   ...
   if(final_transcript.toLowerCase().indexOf("sonraki kutu")>-1  )
  {
	
	recognition.stop();	
	alert("Selam Dünyalı");
	var degisken = 9;
	if (event.keyCode == degisken) {
		$('.voiceWriteHere').focus();
	}
	$('#spch').hide();
	return false;
...
if(final_transcript.toLowerCase().indexOf(key1.toLowerCase())>-1 && key1.toLowerCase()!="" )	  {		
  ...
  if(!mobileRepeatBug) {
	...
	if($('#spchi').height()>128 && r){
	 ...
	 if(final_transcript.toLowerCase().indexOf("satır atla")>-1  ){
